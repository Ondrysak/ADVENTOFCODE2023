use regex::Regex;
use std::collections::HashSet;

fn main() {
    let mut total = 0;
    let input = include_str!("../input_real.txt");
    // Card 1: 41 48 83 86 17 | 83 86  6 31 17  9 48 53
    let length: usize = input.split("\n").count();
    let mut zero_vec: Vec<i32> = vec![1; length];
    let mut i = 0;    
    for line in input.split("\n") {
        println!("{}", line);
        let mut l_total = 1;
        if i == length{
            break;
        }
        let total_cards = zero_vec[i];
        println!("Total card {} count = {}", i+1, total_cards);
        let mut winners: HashSet<_> = HashSet::new();
        let parts: Vec<_>= line.split(":").collect();
        let both = parts[1];
        let b_parts: Vec<_>  = both.split("|").collect();
        let re = Regex::new(r"\d+").unwrap();
        for m in re.find_iter(b_parts[0])
        {
            winners.insert(m.as_str());
        }
        for m in re.find_iter(b_parts[1])
        {
            if winners.contains(m.as_str()){
                if (i+l_total) >= length{
                    continue
                }
                zero_vec[i + l_total] += total_cards;
                println!("Pos {} val {}", i + l_total, zero_vec[i + l_total]);

                l_total += 1;
            }
        }
        total += l_total;
        i += 1;

    }
    for (i,v) in zero_vec.iter().enumerate(){
        println!("Pos {} {}", i+1, v);
    }
    println!("Final total {}", zero_vec.iter().sum::<i32>());
}